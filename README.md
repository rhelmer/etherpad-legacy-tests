Tests for Mozilla's etherpad-legacy install (etherpad.mozilla.org)
====================

Thank you for checking out Mozilla's Etherpad-Legacy test suite. Mozilla and the Mozwebqa team are grateful for the help and hard work of many contributors like yourself.
The following contributors have submitted pull requests to etherpad-legacy-tests:

https://github.com/rhelmer/etherpad-legacy-tests/contributors

Getting involved as a contributor
------------------------------------------

We love working with contributors to fill out the Selenium test coverage for Etherpad-Legacy-Tests, but it does require a few skills.   You will need to know some Python, some Selenium and you will need some basic familiarity with Github.
...
How to Set up and Build Etherpad-Legacy Tests Locally
-----------------------------------------
This repository contains Selenium tests used to test the website etherpad-dev.allizom.org.
...
An example of running all of the tests in one file:
	...
	py.test --driver=firefox --credentials=./credentials.yaml tests/test_create_team_pad.py
	
An example of running one test in a file:

	py.test --driver=firefox --credentials=./credentials.yaml tests/test_create_team_pad.py -k test_that_user_can_create_team_pad
